...
export async function POST(
  ...
  try {
    ...
    const originalPlaybook = mockPlaybooks.find(p => p.id === params.id);
    ...
    const duplicatedPlaybook = {
      ...originalPlaybook,
      id: newId,
      name: newName || `${originalPlaybook.name} - نسخة`,
      ...
      updatedAt: new Date().toISOString(),
      owner: 'current-user', // Current user becomes owner
    };

    // Add unique IDs to steps
    duplicatedPlaybook.steps = duplicatedPlaybook.steps.map((step, index) => ({
      ...step,
      id: `step-${newId}-${index + 1}`,
    }));

    // Add to mock data (in real app, save to database)
    mockPlaybooks.push(duplicatedPlaybook);

    // Simulate network delay
    await new Promise(resolve => setTimeout(resolve, 400));

    return NextResponse.json(duplicatedPlaybook, { status: 201 });
  } catch (error) {
    console.error('Error duplicating playbook:', error);
    return NextResponse.json(
      { error: 'فشل في نسخ الـ Playbook' },
      { status: 500 }
    );
  }
}